...
const Toggle = ({setSwitchState, switchState, titleOne, titleTwo, containerStyle}: IToggle) => {
    return (
        <View style={[{ justifyContent: "center", alignItems: "center", borderRadius: 50, flexDirection: "row" }, containerStyle]} >
                    <TouchableOpacity onPress={() => setSwitchState(true)} style={{ backgroundColor: switchState ? "#216B36" : "#FFFFFF", borderRadius: 25, paddingHorizontal: 20, paddingVertical: 6 }}>
                        <AnicureText text={titleOne} type="subTitle" otherStyles={{ color: switchState ? "#FFFFFF" : "#216B36", fontFamily: "Roboto-Medium" }} />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => setSwitchState(false)} style={{ backgroundColor: switchState ? "#FFFFFF" : "#216B36", paddingHorizontal: 30, borderRadius: 25, paddingVertical: 6 }}>
